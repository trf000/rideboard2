 ...
 *
 * @version $Id: Ride.php 7776 2010-06-11 13:52:58Z jtickle $
...
class RB_Ride {
    ...
    public function getDepartTime()
    {
        if (time() > $this->depart_time) {
            return sprintf('%s (%s)', strftime('%d %b, %Y HH:MM', $this->depart_time),
            dgettext('rideboard', 'Expired'));
        } else {
            return strftime('%d %B, %Y HH:MM', $this->depart_time);
        }
    }
    ...
    public function view()
    {
        ...
        $tpl['EMAIL'] = sprintf('<a href="mailto:%s" class="btn btn-default">%s</a>',
        $user->getEmail(),
